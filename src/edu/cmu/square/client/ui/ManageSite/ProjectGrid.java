...
public class ProjectGrid extends Composite implements Command
{
		...
		manageSiteService.getAllUsers(new AsyncCallback<List<GwtUser>>()
			{
				...
				public void onSuccess(List<GwtUser> result)
				{
					...
					{
						CreateProjectDialog dialog = new CreateProjectDialog(project.getId(), project.getName(),project.getacquisitionOrgEngineer().getUserId(), listOfProjects, result, caller, project.getCases().getId());
						dialog.center();
	...
	private void updateProjectToTable(int row, final GwtProject project)
	{
		...
		this.projectTable.setWidget(row, 2, new Label(project.getacquisitionOrgEngineer().getFullName()));
